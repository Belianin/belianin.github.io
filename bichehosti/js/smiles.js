...
function convertToHtml(smile) {
    ...
    <section class="term-section centered">
        ...
        <div style="width: ${50 * countRating(smile)}px; height: 16px; background-color: indigo"></div><span>${countRating(smile)}</span>
        <div class="history">
            <i>${smile.history}</i>
        </div>
    </section>`
...
function onLoad() {
    const result = [];
    const smiles = tempSmiles;//JSON.parse(loadFile("bichehosti/data/smiles.json"));
